// Command_VariableOperation.h - 提供ImUserWidgetClass的统一变量操作命令
#pragma once
...
enum class VariableOperationSubType
{
    ...
    PasteVariable,            // 粘贴变量（通过json数据）
    PasteObjectVariable,      // 粘贴ImObject的Json数据为ImObject变量
    PasteWidgetVariable       // 粘贴ImWidget的Json数据为控件变量
};

class VariableOperationCommandBase : public ImUserWidgetClassCommandBase
{
protected:
    std::string m_VariableName;  // 执行后生成的变量名
...
private:
    ...
    std::string m_SpecificType;  // 具体类型字符串
...
private:
    ImGuiWidget::WidgetClassVariableType m_Type;
    std::string m_SpecificType;      // 具体类型字符串
    nlohmann::json m_SerializedData; // 序列化的变量数据，用于恢复
    ...
    virtual bool Undo() override
    {
        ...
        case ImGuiWidget::WidgetClassVariableType::Basic:
        {
            ...
            if (basicVar)
            {
                ...
                // 注意：SetBasicVariableDirect会接管所有权，因此var指针不应再被删除
                var = nullptr;  // 防止后续delete
            }
            break;
        }
        }
...
// ============================================================================
// 粘贴变量命令（通过ImWidgetClassVariable序列化得到的json数据）
// ============================================================================
class PasteVariableCommand : public VariableOperationCommandBase
{
private:
    nlohmann::json m_SerializedData;      // 序列化的变量数据
    std::string m_OriginalVariableName;   // 原始变量名（来自复制源）
    ...
    virtual bool Execute() override
    {
        ...
        case ImGuiWidget::WidgetClassVariableType::Basic:
        {
            ...
            if (basicVar)
            {
                ...
                // SetBasicVariableDirect会接管所有权，因此var指针不应再被删除
                var = nullptr;  // 防止后续delete
            }
            break;
        }
...
// ============================================================================
// 粘贴ImObject的Json数据为ImObject变量
// ============================================================================
class PasteObjectVariableCommand : public VariableOperationCommandBase
{
private:
    nlohmann::json m_ObjectJson;        // ImObject的Json数据（通过SerializeImObject得到）
...
// ============================================================================
// 粘贴ImWidget的Json数据为控件变量
// ============================================================================
class PasteWidgetVariableCommand : public VariableOperationCommandBase
{
private:
    nlohmann::json m_WidgetJson;        // ImWidget的Json数据（通过SerializeImWidget得到）
    std::string m_SuggestedName;        // 建议的变量名
    bool m_KeepSuggestedName;           // 是否保持建议名称
...
public:
    ...
    virtual std::string GetDescription() const override
    {
        ...
        return "粘贴控件变量: " + m_VariableName + " (类型: " + typeName + ")";
    }
