...
namespace ImGuiWidget
{
	...
	private:
		//void* m_dragResource;
		static std::shared_ptr<ImDragObject> GlobalDragObject;
	...
	public:
		...
		// 获取拖拽数据
		std::shared_ptr<ImDragObject> GetDragObject() const { return GlobalDragObject; }
		ImVec2 GetPosition() const { return m_dragPosition; }
		ImVec2 GetDelta() const { return m_dragDelta; }

		// 设置拖拽数据
		void SetDragData(std::shared_ptr<ImDragObject> resource)
		{
			GlobalDragObject = resource;
		}
		static void ClearDragData()
		{
			GlobalDragObject.reset();
		}
		static ImWidget* GetDragPreview()
		{
			if (GlobalDragObject)
			{
